...
int md::memory_map()
{
	...
	if (rommax > 0)
	{
		fetch[i].lowaddr = 0x000000;
		fetch[i].highaddr = rommax - 1;
		fetch[i].offset = (unsigned)rom - 0x000000;
		i++;
	}
	fetch[i].lowaddr = 0xff0000;
	fetch[i].highaddr = 0xffffff;
	fetch[i].offset = (unsigned)ram - 0xff0000;
	i++;
	// Testing
	fetch[i].lowaddr = 0xffff0000;
	fetch[i].highaddr = 0xffffffff;
	fetch[i].offset = (unsigned)ram - 0xffff0000;
	i++;
	// Testing 2
	fetch[i].lowaddr = 0xff000000;
	fetch[i].highaddr = 0xff000000 + rommax - 1;
	fetch[i].offset = (unsigned)rom - 0xff000000;
	i++;
	fetch[i].lowaddr = fetch[i].highaddr = 0xffffffff;
	fetch[i].offset = 0;
	i++;
...
#endif
	if (debug_log)
		fprintf(debug_log, "reset()\n");

	aoo3_toggle = aoo5_toggle = aoo3_six = aoo5_six = aoo3_six_timeout = aoo5_six_timeout = coo4 = coo5 = 0;
...
int md::load(const char *name)
{
	...
	// Replace "EMULATOR" with "CHAOSDRV" in ROM data
	// Pointless easter egg, basically.
	bool modified = false;
	for (size_t i = 0; i <= size - 8; i++)
	{
		if (memcmp(temp + i, "EMULATOR", 8) == 0)
		{
			memcpy(temp + i, "CHAOSDRV", 8);
			modified = true;
		}
	}

	// If we modified the ROM data, recalculate the checksum
	if (modified && size >= 0x190)
	{
		// Calculate checksum (sum of all 16-bit words from 0x200 onwards)
		unsigned short cs = 0;
		for (size_t i = 0x200; i < size - 1; i += 2)
		{
			cs += (temp[i] << 8) + temp[i + 1];
		}
		temp[0x18e] = cs >> 8;
		temp[0x18f] = cs & 255;
		printf("ChaosDrive: Updated ROM checksum to 0x%04x\n", cs);
		// Update the header info we already copied
		cart_head.checksum = cs;
	}
...
{
	unsigned short cs;
	cs = calculate_checksum(rom, romlen);
	if (romlen >= 0x190)
	{
		rom[ROM_ADDR(0x18e)] = cs >> 8;
		rom[ROM_ADDR(0x18f)] = cs & 255;
	}
}
